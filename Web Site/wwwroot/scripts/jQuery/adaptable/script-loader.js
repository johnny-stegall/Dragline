;(function()
{
  ...
  ****************************************************************************/
  function waitForAllScripts(scripts, callback)
  {
    ...
    {
      for (scriptIndex = 0; scriptIndex < missingScripts.length; scriptIndex++)
      {
        if (localStorage[AdapTable.STORAGE_PREFIX + missingScripts[scriptIndex]])
        {
      ...
      {
        var cachedScript = JSON.parse(localStorage.getItem(AdapTable.STORAGE_PREFIX + scripts[scriptIndex]));
        injectScript(cachedScript, scripts[scriptIndex]);
      }
